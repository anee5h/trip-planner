      ...
      <div className="flex flex-wrap gap-2 border-t border-slate-100 dark:border-slate-800 pt-4">
        {[...STATUSES, "ALL" as const].map((status) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-xl text-xs font-bold ${filter === status ? "bg-slate-900 text-white dark:bg-emerald-500 dark:text-slate-950" : "bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400"}`}
          >
            {status.replace("_", " ")} (
              ...
              <div className="min-w-0">
                ...
                {website && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center gap-1 mt-2 text-xs text-emerald-600 dark:text-emerald-400 max-w-full"
                  >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-2 gap-1.5">
                {STATUSES.map((nextStatus) => (
                  <button
                    ...
                    className={`px-2 py-1.5 rounded-lg text-[10px] font-extrabold border ${status === nextStatus ? "bg-emerald-600 text-white border-emerald-500" : "bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-700"}`}
                  >
