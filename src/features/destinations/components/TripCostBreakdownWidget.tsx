          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap mb-1">
              <h3 className="text-xl font-extrabold text-slate-900 dark:text-white flex items-center gap-2">
                ...
                {locale === "ja" ? "料金・費用内訳" : "Itemized Cost Breakdown"}
              </h3>
              {cost.confidence === "high" && (
                <Badge className="bg-emerald-50 text-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-300 border-emerald-200 dark:border-emerald-800 text-[10px] font-bold">
                  {locale === "ja" ? "公式料金データ" : "Curated Fares"}
                </Badge>
              )}
            </div>
            <p className="text-xs text-slate-500 dark:text-slate-400">
              {locale === "ja"
                ? "交通費・入場料・食事・カフェ・駐車料金の概算内訳"
                : "Estimated breakdown for transport, tickets, dining, café, and parking."}
            </p>
          </div>

          {/* Toggle Per-Person vs. Party */}
          <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-lg self-end sm:self-auto text-xs font-semibold">
            <button
        ...
        <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-200/80 dark:border-slate-700/80">
          <div>
            ...
            <div className="text-2xl font-extrabold text-slate-900 dark:text-white mt-0.5">
              {formatLocalizedJPYRange(
                viewMode === "party" ? cost.partyRange : cost.perPersonRange,
                locale,
              )}
            </div>
          </div>
              ...
              <span className="text-slate-900 dark:text-white">
                {formatLocalizedJPYRange(
                  viewMode === "party"
                    ...
                    : [cost.transport / partySize, cost.transport / partySize],
                  locale,
                )}
              </span>
            </div>
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between text-xs font-bold">
              <span className="text-slate-700 dark:text-slate-300 flex items-center gap-1.5">
                ...
                {locale === "ja" ? "入場チケット・拝観料" : "Admission Tickets"}
              </span>
              <span className="text-slate-900 dark:text-white">
                {cost.isFreeTicket
                  ? locale === "ja"
                  ...
                  : formatLocalizedJPYRange(
                      viewMode === "party"
                        ...
                        : [cost.tickets / partySize, cost.tickets / partySize],
                      locale,
                    )}
              </span>
            </div>
                ...
                <span className="text-slate-900 dark:text-white">
                  {formatLocalizedJPYRange(
                    viewMode === "party"
                      ...
                      : [cost.parking / partySize, cost.parking / partySize],
                    locale,
                  )}
                </span>
              </div>
