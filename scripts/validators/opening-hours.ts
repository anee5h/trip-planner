import type { ValidatorModule } from "./types";
import { getOpeningHoursAssessment } from "../../src/shared/services/recommendation/OpeningHoursPolicy";
import type { Destination } from "../../src/shared/types/destination";
import openingHoursAllowlistJson from "./opening-hours-allowlist.json";

/**
 * Kinds that describe open areas rather than a single gated facility.
 * A specific window ("09:00 - 17:00") claimed for these kinds is
 * semantically suspect unless it is verified to a real facility.
 */
const OPEN_AREA_KINDS = new Set([
  "nature",
  "beach",
  "lake",
  "park",
  "mountain",
  "viewpoint",
  "waterfall",
  "island",
  "cape",
  "cliff",
  "rock_formation",
  "onsen",
  "district",
  "street",
  "ward",
  "town",
  "village",
  "historic",
  "historic_town",
  "garden",
  "bridge",
  "entertainment",
]);

// Anchored window: "09:00 - 17:00", "09:00–17:00", "08:30—17:00" at the
// start of the hours string (ASCII hyphen, en dash, em dash). Unanchored
// prose ("Open access; facility hours…") is validated by humans, not this
// gate.
const SPECIFIC_WINDOW = /^\d{1,2}:\d{2}\s*[-\u2013\u2014]\s*\d{1,2}:\d{2}/;
const HTTP_URL = /^https?:\/\/\S+$/i;

function loadAllowlist(): Set<string> {
  return new Set(openingHoursAllowlistJson);
}

function hasFreshMetadata(
  destination: Destination,
  now: Date = new Date(),
): boolean {
  const meta = destination.openingHoursMetadata;
  if (!meta?.sourceUrl || !meta.verifiedAt) return false;
  const verifiedDate = new Date(meta.verifiedAt);
  if (Number.isNaN(verifiedDate.getTime())) return false;
  const ageDays =
    (now.getTime() - verifiedDate.getTime()) / (1000 * 60 * 60 * 24);
  return ageDays >= 0 && ageDays <= 180;
}

/**
 * Core validation with an explicit allowlist (injectable for tests).
 */
export function validateOpeningHours(
  context: Parameters<ValidatorModule["validate"]>[0],
  allowlist: Set<string>,
) {
  const activeAllowlistedIds = new Set<string>();
  const issues: Array<{
    severity: "error" | "warning" | "info";
    code: string;
    message: string;
    targetId?: string;
  }> = [];
  let warnings = 0;
  let infos = 0;

  for (const destination of context.catalog.destinations) {
    const id = destination.id;
    // The policy treats either field as hours; the gate must too.
    const hours = destination.businessHours ?? destination.openingHours;
    const meta = destination.openingHoursMetadata;
    const assessment = getOpeningHoursAssessment(destination);

    // Malformed verification metadata is always an error (new defect).
    if (meta?.verifiedAt) {
      const verifiedDate = new Date(meta.verifiedAt);
      if (
        Number.isNaN(verifiedDate.getTime()) ||
        verifiedDate.getTime() > new Date().getTime()
      ) {
        issues.push({
          severity: "error",
          code: "MALFORMED_HOURS_METADATA",
          message: `Destination '${id}' has an invalid or future openingHoursMetadata.verifiedAt (${meta.verifiedAt}).`,
          targetId: id,
        });
      }
    }

    // Verification sources must be real http(s) URLs.
    if (meta?.sourceUrl && !HTTP_URL.test(meta.sourceUrl)) {
      issues.push({
        severity: "error",
        code: "INVALID_HOURS_METADATA_URL",
        message: `Destination '${id}' has a non-http(s) openingHoursMetadata.sourceUrl (${meta.sourceUrl}).`,
        targetId: id,
      });
    }

    if (
      assessment.status !== "not_required" &&
      OPEN_AREA_KINDS.has(destination.kind ?? "") &&
      hours &&
      SPECIFIC_WINDOW.test(hours) &&
      !hasFreshMetadata(destination)
    ) {
      if (allowlist.has(id)) {
        activeAllowlistedIds.add(id);
        warnings += 1;
        issues.push({
          severity: "warning",
          code: "OPEN_AREA_SPECIFIC_WINDOW_UNVERIFIED",
          message: `Destination '${id}' (${destination.kind}) carries specific-window hours '${hours}' with no fresh verification; allowlisted debt (see issue #335).`,
          targetId: id,
        });
      } else {
        issues.push({
          severity: "error",
          code: "OPEN_AREA_SPECIFIC_WINDOW_UNVERIFIED",
          message: `Destination '${id}' (${destination.kind}) carries specific-window hours '${hours}' with no fresh verification metadata. Verify via a source URL (openingHoursMetadata.sourceUrl + verifiedAt) or use open-area wording.`,
          targetId: id,
        });
      }
      continue;
    }

    if (
      hours &&
      !OPEN_AREA_KINDS.has(destination.kind ?? "") &&
      meta &&
      (!meta.sourceUrl || !meta.verifiedAt) &&
      assessment.status === "unverified"
    ) {
      warnings += 1;
      issues.push({
        severity: "warning",
        code: "HOURS_METADATA_INCOMPLETE",
        message: `Destination '${id}' has openingHoursMetadata without sourceUrl+verifiedAt; hours remain unverified.`,
        targetId: id,
      });
    }

    if (hours && !meta && assessment.status !== "not_required") {
      infos += 1;
      issues.push({
        severity: "info",
        code: "HOURS_WITHOUT_METADATA",
        message: `Destination '${id}' shows businessHours with no verification metadata.`,
        targetId: id,
      });
    }
  }

  // KAI-335 hardening: allowlist entries are enforced debt, not free
  // passes. An ID that no longer matches an active violation must be
  // removed from the allowlist by the PR that repaired it.
  for (const id of allowlist) {
    if (!activeAllowlistedIds.has(id)) {
      issues.push({
        severity: "error",
        code: "STALE_OPENING_HOURS_ALLOWLIST_ENTRY",
        message: `Allowlist entry '${id}' no longer matches an active OPEN_AREA_SPECIFIC_WINDOW_UNVERIFIED violation; remove it from opening-hours-allowlist.json.`,
        targetId: id,
      });
    }
  }

  const errors = issues.filter((i) => i.severity === "error");
  return {
    name: "Opening Hours Integrity",
    passed: errors.length === 0,
    issues,
    metrics: {
      totalChecked: context.catalog.destinations.length,
      errorsCount: errors.length,
      warningsCount: warnings,
      infoCount: infos,
      durationMs: 0,
    },
  };
}

export const openingHoursValidator: ValidatorModule = {
  name: "Opening Hours Integrity",
  description:
    "Validates businessHours shape, kind fit, and verification metadata for the catalogue.",
  purpose:
    "Prevent blanket or unverified opening windows on open-area destinations and malformed hours metadata.",
  guarantees: [
    "Specific-window businessHours/openingHours on open-area kinds carry fresh metadata or are allowlisted debt",
    "Every allowlisted debt entry still matches an active violation (stale entries fail)",
    "openingHoursMetadata.verifiedAt is a valid past date when present",
    "openingHoursMetadata.sourceUrl is an http(s) URL when present",
  ],
  doesNotValidate: [
    "Editorial truthfulness of verified hours (source review is human)",
    'Unanchored prose formats ("Open access; facility hours…") that describe sub-areas',
  ],
  async validate(context) {
    return validateOpeningHours(context, loadAllowlist());
  },
};
