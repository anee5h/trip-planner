...
const index = JSON.parse(fs.readFileSync(path.join(rootDir, "src/shared/data/destinations-index.json"), "utf8")) as Destination[];
const byId = new Map(index.map((d) => [d.id, d]));
const truth = loadTruth();
const audit = JSON.parse(fs.readFileSync(path.join(rootDir, "scripts/audit/kai-89-structured-template-audit.json"), "utf8")) as {
  categories: Array<{ category: string; clusters: Array<{ id: string; ids: string[]; count: number; value: unknown }> }>;
};
const corr = JSON.parse(fs.readFileSync(path.join(rootDir, "scripts/audit/kai-89-corrections.json"), "utf8")) as {
  sections: Record<string, Array<{ id: string }> | undefined>;
};
const A = JSON.parse(fs.readFileSync("/tmp/kai89-audit/AgentA-ratings.json", "utf8"));
const B = JSON.parse(fs.readFileSync("/tmp/kai89-audit/AgentB-budget.json", "utf8"));
const C = JSON.parse(fs.readFileSync("/tmp/kai89-audit/AgentC-transport.json", "utf8"));
const D = JSON.parse(fs.readFileSync("/tmp/kai89-audit/AgentD-season-crowd-duration.json", "utf8"));

const idsOf = (list: Array<{ id: string } | string> | undefined) =>
  new Set((list ?? []).filter((i) => typeof i === "string" || i?.id).map((i) => (typeof i === "string" ? i : i.id)));
const correctedByCat: Record<string, Set<string>> = {
  ratings: idsOf(corr.sections.ratingMetadataDowngrade),
  budget: new Set([...idsOf(corr.sections.budgetTicketCorrections), ...idsOf(corr.sections.budgetRebalanceOnly)]),
...
const agentClusters: Record<string, Array<{ ids: Set<string>; category: string; reason: string }>> = {
  ratings: A.perClusterClassification.map((c: any) => ({ ids: new Set(c.ids ?? c.affectedIds ?? []), category: c.category, reason: (c.reason ?? "").slice(0, 300) })),
  budget: B.clusterClassification.map((c: any) => ({ ids: new Set(c.ids ?? []), category: "B", reason: (c.rootCause ?? c.editorialNote ?? "").slice(0, 300) })),
  transport: C.clusters.map((c: any) => ({ ids: new Set(c.ids.map((r: any) => (typeof r === "string" ? r : r.id))), category: c.class ?? "E", reason: (c.rootCause ?? c.rationale ?? "").slice(0, 300) })),
};
for (const cat of ["season", "visitDuration", "crowd", "comfort", "walking"]) {
  agentClusters[cat] = (D.subcategories[cat] ?? []).map((c: any) => ({ ids: new Set(c.ids ?? []), category: c.category ?? "E", reason: (c.rootCause ?? c.note ?? "").slice(0, 300) }));
}
...
const dispositions = {
  ...
  clusters: {} as Record<string, { category: string; action: string; reason: string }>,
};
let unmatched = 0;
let modelClusters = 0;
for (const cat of audit.categories) {
  ...
  for (const cl of cat.clusters) {
    ...
    if (isModelCluster(cat.category, cl.ids)) {
      entry = { category: "C", action: "model-estimate", reason: "KAI-89 model estimate: all members derived by the documented model (scripts/models/DESIGN.md); low-confidence provenance in editorial.fieldSources" };
      modelClusters++;
    } else {
      ...
      if (best && bestOverlap >= Math.min(3, currentIds.size) && bestOverlap >= 0.5 * currentIds.size) {
        const hasCorrection = [...currentIds].some((id) => catCorrected.has(id));
        let action: string;
        ...
        entry = { category: "E", action: "manual-review", reason: "unclassified by KAI-89 audit agents; requires editorial review" };
        unmatched++;
      }
    }
...
fs.writeFileSync(path.join(rootDir, "scripts/audit/kai-89-dispositions.json"), JSON.stringify(dispositions, null, 1));
const total = audit.categories.reduce((s, c) => s + c.clusterCount, 0);
const byAction: Record<string, number> = {};
for (const e of Object.values(dispositions.clusters)) byAction[e.action] = (byAction[e.action] ?? 0) + 1;
console.log(`dispositions: ${Object.keys(dispositions.clusters).length}/${total} | model-estimate: ${modelClusters} | unmatched: ${unmatched} |`, JSON.stringify(byAction));
