 ...
 * destinations-meta.json, destinations-index.lite.json, and the compact
 * destination-relationships.json from the index
...
export function outputKeyDisplayName(key: string): string {
  ...
  if (key === "relationship-index") {
    return "src/shared/data/destination-relationships.json";
  }
...
export function toOutputMap(o: {
  ...
  map.set("relationship-index", o.relationshipIndex);
  return map;
}
...
export function loadCommittedOutputs(rootDir: string): Map<string, string> {
  ...
  committed.set(
    "relationship-index",
    fs.readFileSync(relationshipIndexPath, "utf-8"),
  );
  return committed;
}
...
export async function runSyncCheck(
  ...
  console.log(
    `Generated ${firstGeneration.detailFiles.size} detail files + meta + client index + relationship index from the index (run 1 of 2).`,
  );
