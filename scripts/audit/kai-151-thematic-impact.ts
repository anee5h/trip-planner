...
const config: Record<
  string,
  { review: string; output: string; peak: string; off: string }
> = {
  "2B": {
    review: "scripts/audit/kai-151-foliage-phase2b-review.json",
    output: "scripts/audit/kai-151-foliage-phase2b-impact.json",
    peak: "2026-11-10",
    off: "2026-07-20",
  },
  "2C": {
    review: "scripts/audit/kai-151-winter-phase2c-review.json",
    output: "scripts/audit/kai-151-winter-phase2c-impact.json",
    peak: "2026-01-20",
    off: "2026-07-20",
  },
  "2D": {
    review: "scripts/audit/kai-151-alpine-phase2d-review.json",
    output: "scripts/audit/kai-151-alpine-phase2d-impact.json",
    peak: "2026-08-15",
    off: "2026-01-20",
  },
  "2E": {
    review: "scripts/audit/kai-151-summer-phase2e-review.json",
    output: "scripts/audit/kai-151-summer-phase2e-impact.json",
    peak: "2026-07-20",
    off: "2026-01-20",
  },
};
...
const before = JSON.parse(
  execFileSync(
    "git",
    ["show", `${review.baseCommit}:src/shared/data/destinations-index.json`],
...
const ids = (review.records as JsonObject[])
...
function context(
  ...
  return {
    ...
    partySize: 2,
    visitedIds: [],
    homeStationCoords: origin.coords,
    ...
    for (const duration of durations) {
      ...
      for (const id of ids) {
        ...
        const scoreDelta = round(
          (afterRow?.score ?? 0) - (beforeRow?.score ?? 0),
        );
        const seasonalDelta = round(
          afterSeason.scoreDelta - beforeSeason.scoreDelta,
        );
        rows.push({
          phase,
          id,
          ...
          rankDelta:
            beforeRank !== null && afterRank !== null
              ...
              : null,
          beforeScore: beforeRow ? round(beforeRow.score) : null,
          afterScore: afterRow ? round(afterRow.score) : null,
          scoreDelta,
          beforeSeasonalDelta: round(beforeSeason.scoreDelta),
          afterSeasonalDelta: round(afterSeason.scoreDelta),
          seasonalDelta,
          beforeSeasonalEvidence: beforeSeason.evidence,
          afterSeasonalEvidence: afterSeason.evidence,
...
const output = {
  ...
  scenarioDefinition:
    "3 representative origins × 5 planner durations × one in-season and one materially off-season date; existing recommendation pipeline only",
  targetIds: ids,
  summary: {
...
writeFileSync(
  resolve(root, config[phase].output),
...
const md = [
  ...
  "| Classification | Rows |",
  "| --- | ---: |",
  ...Object.entries(output.summary.classifications).map(
    ([key, value]) => `| ${key} | ${value} |`,
  ),
  "",
  "## Method",
...
console.log(
  `KAI-151 Phase ${phase} impact rows=${rows.length} classifications=${JSON.stringify(output.summary.classifications)}`,
);
