...
const rows = list
  .map((d) => ({
    d,
    a: getOpeningHoursAssessment(d as never, new Date("2026-09-05T00:00:00Z")),
  }))
  .map(({ d, a }) => ({
    id: d.id,
    name: d.name,
...
function hasFreshMetadata(d: {
  ...
  return (Date.now() - date.getTime()) / (1000 * 60 * 60 * 24) <= 180;
}
...
for (const r of rows) {
  if (
    ...
    r.displayText &&
    SPECIFIC_WINDOW.test(r.displayText) &&
    !hasFreshMetadata(r)
  ) {
...
const report = {
  ...
  systemicCauses: [
    ...
    "673 records carry hours with no openingHoursMetadata, leaving status unverified with no stale/verified distinction",
...
const md = [
  ...
  "## Findings",
  "",
  `1. **Suspicious cohort (${suspiciousCohort.length})** — specific-window hours on open-area kinds with no metadata: ${report.cohorts.suspicious.join(", ")}`,
  ...
  `3. **Verified only ${statusCounts.verified}** of ${report.input.records} records → 96.6% of records show hours without fresh evidence.`,
  `4. **Sourced grants** ${statusCounts.sourced} records — policy counts any official website as hours evidence (conflation).`,
  `5. **No CI validator** touches businessHours/openingHours (checked scripts/cli/*, audit-catalog-integrity, validate-all).`,
  `6. **Detail page shows raw hours as fact**; only the plan widget discloses unverified status.`,
  "",
  "## Prevention (ordered)",
  "",
  "1. CI validator gate: specific-window hours on open-area kinds must carry fresh metadata (sourceUrl + verifiedAt).",
  "2. Tighten `sourced`: require `openingHoursMetadata.sourceUrl`; official website alone must not certify hours.",
  "3. Detail page: append 'not yet verified' caveat when assessment is stale/unverified (mirror DayPlanWidget).",
  "4. Repair cohort source-backed: nonsense IDs → kind-appropriate 'Open access (…)' text or verified metadata.",
  "5. Reuse the 2026-08 batch verification process for the 673-record unverified remainder (long tail).",
  "",
].join("\n");
