...
1. A gateway relationship means **actual access**, not containment: `Nara` is mapped to Nara Station, not to Kyoto Station as its "shinkansen gateway" — Kyoto is a _transfer_ gateway, and any claim "Shinkansen to Nara" must model the Kyoto→Nara leg explicitly.
