...
⚠️ Sapporo→Hokkaido-prefecture shinkansen `verified=121` relies on the `tokyo→hokkaido`-style registry overgeneralization (see architecture doc §2.1): the prefecture-pair registry can return a shinkansen duration for a _local_ Hokkaido destination even though the Hokkaido Shinkansen only reaches Shin-Hakodate-Hokuto today.
