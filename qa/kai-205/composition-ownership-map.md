...
| More things to do (hub)         | `indoorChildren` and `foodAndEveningChildren` in `DestinationDetails`; each source is sorted and sliced to 3, then locally deduped and featured IDs removed          |                           At most 6 before featured exclusion; rail has no extra slice | Child relationship, locale, indoor/category filters; no cross-rail claim against other sections                                               | `DestinationDetailRail` + `DestinationCard`                                                              |
| Great additions to this trip    | `findNearbyCombinations(destination, undefined, 3)` → `DestinationCombinationRail`                                                                                   | Combination service ranks candidate tiers/rating/distance, category-diversifies, max 3 | Local transit/coordinates, visit-time ≤ 600 min, combination semantics; no active Home TripContext budget/transport filtering in current call | `DestinationCombinationRail` own combination card family; local duplicate filter only                    |
| Nearby Places                   | `DestinationRelationshipService.getNearbyDestinations`                                                                                                               |                          Explicit parent/nearby relationships or siblings, sliced to 4 | Relationship/index and locale availability                                                                                                    | `DestinationDetailRail` + `DestinationCard`; participates in ordered cross-rail canonical-ID composition |
| More half-day options           | Parent children filtered to `recommendedVisitHours.max <= 4`, then sliced to 3                                                                                       |                                                                  Parent-child pool / 3 | Parent relation, visit-hour bound, current-place exclusion, locale                                                                            | `DestinationDetailRail` + `DestinationCard`; hidden when its composed pool is empty                      |
| Nearby hubs                     | `DestinationRelationshipService.getNearbyHubs(destination, 50)`                                                                                                      |                                All hubs within 50 km; no render cap in current service | Hub role, coordinates, radius, locale                                                                                                         | `DestinationDetailRail` + `DestinationCard`; rendered in hub Go Next                                     |
...
- Great additions: `ghibli-museum`, `jindaiji`, `kitaro-chaya`;
- Nearby places: `chofu-tokyo`, `jindai-botanical-gardens`;
- More half-day options: hidden because its remaining eligible pool is empty.
