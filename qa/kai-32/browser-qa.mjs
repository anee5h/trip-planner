...
for (const c of CASES) {
  for (const locale of ["en", "ja"]) {
    for (const width of [1440, 410]) {
      ...
      page.on("console", (m) => {
        if (m.type() === "error" && !/favicon/.test(m.text())) errors.push(`console: ${m.text().slice(0, 120)}`);
      });
      try {
        ...
        await page.waitForSelector("text=/Add to Itinerary|旅程に追加/", { timeout: 20000 });
        await page.waitForTimeout(800);
      } catch (e) {
        results.push({ ...c, locale, width, ok: false, errors: [`wait: ${e.message.slice(0, 100)}`] });
        await page.close();
        continue;
      }
      const body = (await page.locator("body").innerText()).slice(0, 4000);
      const hasJapanese = /[\u3040-\u30ff\u3400-\u4dbf\u4e00-\u9fff]/.test(body);
      ...
      if (c.id === "miyajima-itsukushima") {
        ...
        if (/naha|nago|motobu|Okinawa|沖縄/.test(body)) note.push("OKINAWA-LEFTOVER");
        if (/train|Train|電車/.test(body)) note.push("TRAIN-CLAIMED");
      }
      if (c.id === "izumo-taisha" && /Rikuch|Iwate|2011|岩手/.test(body)) note.push("IWATE-LEFTOVER");
      if (c.id === "korakuen-okayama" && /1280px/.test(body)) note.push("hero-1280px");
      if (/Not Found|404/.test(body)) note.push("NOT-FOUND");
      if (locale === "ja" && !hasJapanese) note.push("NO-JA");
      results.push({ ...c, locale, width, ok: ok && jaOk, bodyLen: body.length, ja: hasJapanese, note: note.join(",") || "-", errors: errors.slice(0, 2) });
      await page.close();
    }
  }
}
